...
__global__ void ROIPoolForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {//batch*channels*7*7
    // (n, c, ph, pw) is an element in the pooled output
    int pw = index % pooled_width;
    int ph = (index / pooled_width) % pooled_height;
    int c = (index / pooled_width / pooled_height) % channels;
    int n = index / pooled_width / pooled_height / channels;//id in batch
...
void ROIPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* bottom_data = bottom[0]->gpu_data();//batch*channels*height*width
  const Dtype* bottom_rois = bottom[1]->gpu_data();//batch*4
  Dtype* top_data = top[0]->mutable_gpu_data();//batch*channels*7*7
